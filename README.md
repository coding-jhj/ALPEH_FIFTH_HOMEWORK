...
<img src="docs/infographics.png" width="620" alt="관측 시계열, 서버 점유율 게이지, 두 원천 대조, 실패 다섯 갈래 매트릭스">

| 그래프 | 무엇을 말하나 |
|---|---|
| **관측 시계열** | 조회할 때마다 남긴 관측 로그. 일별 저장과 별개 테이블이라 채점 규칙에 영향 없음 |
| **서버 점유율** | `players.online / players.max`. 원천이 주는데 안 쓰던 값을 살렸습니다 |
| **두 원천 대조** | mcstatus.io vs mcsrvstat.us. 같은 사실을 두 곳이 조금 다르게 말합니다 |
| **실패 다섯 갈래** | 각 실패에서 사용자가 취할 다음 행동. 현재 상태 칸이 튀어나옵니다 |

차트 라이브러리를 쓰지 않고 전부 인라인 SVG로 그립니다. 값이 부족하면 억지로 그리지 않고
왜 못 그리는지 문구로 설명합니다.
